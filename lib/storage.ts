...
// ── Catalogue site media ──────────────────────────────────────────────────
// Merged in from the custom-order-requests branch. These serve the public
// catalogue site and are unrelated to the WA_* buckets above: separate
// buckets, separate policies. Rewritten onto storage() rather than the
// module-level client that branch used, so importing this file still does not
// require the Supabase env to be present.

/** Staff-uploaded catalogue post media (photos and video). */
const CATALOGUE_MEDIA_BUCKET = "catalogue-media"

// Separate, policy-constrained bucket for anonymous customer reference-photo
// uploads (custom order requests) — see the catalogue-reference-bucket
// migration. Kept distinct from CATALOGUE_MEDIA_BUCKET (staff-only post
// media) so the anonymous upload path is never widened onto the shared staff
// bucket.
const CATALOGUE_REFERENCE_BUCKET = "catalogue-reference"
 ...
 *  set in the catalogue-reference-bucket migration, IS the real DB-enforced backstop; this cap is a
...
export async function createCatalogueUploadUrl(
  ...
  const { data, error } = await storage().storage.from(CATALOGUE_REFERENCE_BUCKET).createSignedUploadUrl(path)
  if (error) throw new Error(`Failed to create upload URL: ${error.message}`)

  const { data: publicData } = storage().storage.from(CATALOGUE_REFERENCE_BUCKET).getPublicUrl(path)
