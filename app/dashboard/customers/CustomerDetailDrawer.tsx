                  ...
                  {data.payments.map((p) => (
                    <div key={p.rowNumber} className="flex items-center justify-between gap-3 px-4 py-2.5 text-sm">
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        ...
                        <span className={`text-[11px] ${p.isChecked ? "text-green-600" : "text-faint"}`}>
                          {p.isChecked ? "✓" : "pending"}
                        </span>
                      </div>
                    </div>
                  ...
                  {data.adjustments.map((a) => (
                    <div key={a.rowNumber} className="flex items-center justify-between gap-3 px-4 py-2.5 text-sm">
                      <div className="min-w-0">
                        ...
                        {a.description && <div className="text-xs text-faint mt-0.5">{a.description}</div>}
                      </div>
...
function Section({
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      {count === 0 ? (
        <div className="rounded-xl border border-cream-border bg-white px-4 py-5 text-center text-sm text-faint">{empty}</div>
      ) : (
        children
      )}
    </div>
...
function InvoiceBlock({ event }: { event: InvoiceEvent }) {
  ...
  return (
    <div className="rounded-xl border border-cream-border bg-white overflow-hidden">
      ...
      >
        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={`shrink-0 text-faint transition-transform ${open ? "rotate-90" : ""}`}><path d="m9 18 6-6-6-6" /></svg>
        <span className="text-sm font-semibold text-foreground">{event.eventId}</span>
        {event.status && <span className="text-xs text-muted">{event.status}</span>}
          ...
          <div className="divide-y divide-cream-border/60">
            {event.orders.map((o) => (
              <div key={o.orderId} className="flex items-center justify-between gap-3 px-4 py-2 text-sm">
                ...
                <span className="flex items-center gap-3 shrink-0 tabular-nums text-muted">
