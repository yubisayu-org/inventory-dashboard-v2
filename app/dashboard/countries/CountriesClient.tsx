  ...
  return (
    <div className="grid grid-cols-2 gap-3">
      <div className="flex flex-col gap-1">
        <span className="text-xs font-medium text-muted">Live rate</span>
        <div className={`w-full h-[38px] flex items-center border border-cream-border rounded-lg px-3 bg-surface-muted text-muted-strong ${sizeCls(rate)}`}>{body(rate)}</div>
      </div>
      <div className="flex flex-col gap-1">
        <span className="text-xs font-medium text-muted">Markup rate</span>
        <div className={`w-full h-[38px] flex items-center border border-cream-border rounded-lg px-3 bg-surface-muted text-muted-strong ${sizeCls(rate != null ? rate * markup : null)}`}>{body(rate != null ? rate * markup : null)}</div>
      </div>
    </div>
  )
}
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-xs text-faint tabular-nums">· RATE {fmt(row.kurs)}</span>
        </div>
        <div className="text-xs text-faint tabular-nums mt-0.5">
          SHIPPING {fmt(row.cargoPerKg)}/KG
        </div>
      </div>
      <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" className="text-faint shrink-0">
    ...
    <form onSubmit={handleAdd} className="hidden md:flex rounded-xl border border-cream-border bg-white p-5 flex-col gap-4">
      ...
      <div className="flex items-end gap-3">
        <label className="flex-1 min-w-0 flex flex-col gap-1">
          <span className="text-xs font-medium text-muted">Country Name</span>
          <input
            {...field("name")}
        ...
        <label className="flex-1 min-w-0 flex flex-col gap-1">
          <span className="text-xs font-medium text-muted">Shipping / kg (IDR)</span>
          <input
            {...field("cargoPerKg")}
            type="number"
            min="0"
