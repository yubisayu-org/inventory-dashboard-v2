    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="rounded-xl border border-cream-border bg-white overflow-hidden">
        ...
        <div className="border-b border-cream-border bg-cream px-4 py-3 flex gap-6">
          {[40, 80, 60, 70, 50, 60].map((w, i) => (
            <div key={i} className={`h-3 rounded bg-divider animate-pulse`} style={{ width: w }} />
          ))}
        </div>
        ...
        {Array.from({ length: rows }).map((_, i) => (
          <div key={i} className="border-b border-cream-border/60 px-4 py-3 flex gap-6 items-center">
            {[40, 80, 60, 70, 50, 60].map((w, j) => (
              <div
                key={j}
                className="h-3 rounded bg-surface-sunken animate-pulse"
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-1">
          {[1, 2, 3].map((i) => (
            <div key={i} className="h-8 w-8 rounded-lg bg-surface-sunken animate-pulse" />
          ))}
        </div>
      </div>
    </div>
