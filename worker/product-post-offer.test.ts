...
test("askDisambiguation with two candidates lists only the codes, never a bare number", async () => {
  const resolution = await resolveProductPostClaim({
    groupJid: GROUP, messageId: mid("her-1"), sender: HER, text: "bostonnya mau 1 dong", quoted: "",
  })
  assert.equal(resolution.kind, "needsDisambiguation")
  if (resolution.kind !== "needsDisambiguation") return

  const sock = fakeSock(mid("bot-msg-1"))
  ...
  const [row] = await sql`SELECT status, bot_message_id, candidate_send_code_ids FROM catalogue_requests WHERE message_id = ${mid("her-1")}`
  assert.equal(row.status, "asking")
  assert.equal(row.bot_message_id, mid("bot-msg-1"))
...
test("askDisambiguation with one candidate asks a yes/no confirmation", async () => {
  ...
  const [row] = await sql`SELECT status, bot_message_id, candidate_send_code_ids FROM catalogue_requests WHERE message_id = ${mid("her-2")}`
  assert.equal(row.status, "asking")
  assert.equal(row.bot_message_id, mid("bot-msg-2"))
...
test("trySendOfferAnswer settles a code reply against the offered candidates only", async () => {
  const resolution = await resolveProductPostClaim({
    groupJid: GROUP, messageId: mid("her-3"), sender: HER, text: "bostonnya mau 1", quoted: "",
  })
  assert.equal(resolution.kind, "needsDisambiguation")
  if (resolution.kind !== "needsDisambiguation") return
  const sock = fakeSock(mid("bot-msg-3"))
  await askDisambiguation(sock, {
    groupJid: GROUP, messageId: mid("her-3"), sender: HER, text: "bostonnya mau 1", quoted: "",
  }, resolution)

  const offeredCode = (await sql`SELECT sc.code FROM catalogue_requests r JOIN wa_send_codes sc ON sc.id = r.candidate_send_code_ids[1] WHERE r.message_id = ${mid("her-3")}`)[0].code as string

  const emoji = await trySendOfferAnswer({
    groupJid: GROUP, messageId: mid("her-3-reply"), sender: HER, text: `${offeredCode}`, quoted: mid("bot-msg-3"),
  })
  assert.equal(emoji, "📝")

  const [row] = await sql`SELECT status, product_id FROM catalogue_requests WHERE message_id = ${mid("her-3")}`
  assert.equal(row.status, "pending")
})

test("a second answer to an already-resolved offer does nothing", async () => {
  const resolution = await resolveProductPostClaim({
    groupJid: GROUP, messageId: mid("her-4"), sender: HER, text: "bostonnya mau 1", quoted: "",
  })
  if (resolution.kind !== "needsDisambiguation") return
  const sock = fakeSock(mid("bot-msg-4"))
  await askDisambiguation(sock, {
    groupJid: GROUP, messageId: mid("her-4"), sender: HER, text: "bostonnya mau 1", quoted: "",
  }, resolution)
  const offeredCode = (await sql`SELECT sc.code FROM catalogue_requests r JOIN wa_send_codes sc ON sc.id = r.candidate_send_code_ids[1] WHERE r.message_id = ${mid("her-4")}`)[0].code as string

  await trySendOfferAnswer({ groupJid: GROUP, messageId: mid("her-4-a"), sender: HER, text: offeredCode, quoted: mid("bot-msg-4") })
  const emoji = await trySendOfferAnswer({ groupJid: GROUP, messageId: mid("her-4-b"), sender: HER, text: offeredCode, quoted: mid("bot-msg-4") })
...
test("trySendOfferAnswer returns null for a reply quoting no open offer", async () => {
  const emoji = await trySendOfferAnswer({
    groupJid: GROUP, messageId: mid("her-5"), sender: HER, text: "K01", quoted: "not-an-offer",
  })
...
test("trySendOfferThumbsUp settles a one-candidate offer", async () => {
  ...
  const resolution = await resolveProductPostClaim({
    groupJid: GROUP, messageId: mid("her-6"), sender: HER, text, quoted: "",
  })
  assert.equal(resolution.kind, "needsDisambiguation")
  if (resolution.kind !== "needsDisambiguation") return
  assert.equal(resolution.candidates.length, 1, "fixture text is chosen to fuzzy-match exactly one candidate")

  const sock = fakeSock(mid("bot-msg-6"))
  await askDisambiguation(sock, {
    groupJid: GROUP, messageId: mid("her-6"), sender: HER, text, quoted: "",
  }, resolution)

  const emoji = await trySendOfferThumbsUp(GROUP, mid("bot-msg-6"), "👍", HER)
  assert.equal(emoji, "✅")
  const [row] = await sql`SELECT status FROM catalogue_requests WHERE message_id = ${mid("her-6")}`
  assert.equal(row.status, "pending")
})

test("trySendOfferThumbsUp does nothing for a multi-candidate offer", async () => {
  const resolution = await resolveProductPostClaim({
    groupJid: GROUP, messageId: mid("her-7"), sender: HER, text: "bostonnya mau 1", quoted: "",
  })
  if (resolution.kind !== "needsDisambiguation") return
  const sock = fakeSock(mid("bot-msg-7"))
  await askDisambiguation(sock, {
    groupJid: GROUP, messageId: mid("her-7"), sender: HER, text: "bostonnya mau 1", quoted: "",
  }, resolution)
  const emoji = await trySendOfferThumbsUp(GROUP, mid("bot-msg-7"), "👍", HER)
  assert.equal(emoji, null)
})
...
test("trySendOfferThumbsUp ignores a reaction that isn't a positive answer (a 👎, or a removed reaction reported as empty text)", async () => {
  const text = "kuni nya mau 1"
  const resolution = await resolveProductPostClaim({
    groupJid: GROUP, messageId: mid("her-8"), sender: HER, text, quoted: "",
  })
  if (resolution.kind !== "needsDisambiguation") return
  const sock = fakeSock(mid("bot-msg-8"))
  await askDisambiguation(sock, {
    groupJid: GROUP, messageId: mid("her-8"), sender: HER, text, quoted: "",
  }, resolution)

  assert.equal(await trySendOfferThumbsUp(GROUP, mid("bot-msg-8"), "👎", HER), null, "a negative reaction must not settle it")
  assert.equal(await trySendOfferThumbsUp(GROUP, mid("bot-msg-8"), "", HER), null, "a removed reaction (empty text) must not settle it")

  const [row] = await sql`SELECT status FROM catalogue_requests WHERE message_id = ${mid("her-8")}`
...
test("trySendOfferThumbsUp does nothing for somebody else's thumb on her offer", async () => {
  ...
  const text = "kuni nya mau 1"
  const resolution = await resolveProductPostClaim({
    groupJid: GROUP, messageId: mid("her-9"), sender: HER, text, quoted: "",
  })
  if (resolution.kind !== "needsDisambiguation") return
  const sock = fakeSock(mid("bot-msg-9"))
  await askDisambiguation(sock, {
    groupJid: GROUP, messageId: mid("her-9"), sender: HER, text, quoted: "",
  }, resolution)

  const emoji = await trySendOfferThumbsUp(GROUP, mid("bot-msg-9"), "👍", OTHER)
  assert.equal(emoji, null, "a stranger's thumb must not settle another customer's claim")
  const [row] = await sql`SELECT status FROM catalogue_requests WHERE message_id = ${mid("her-9")}`
  assert.equal(row.status, "asking")
})
