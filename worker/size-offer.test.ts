...
test("a size she offered herself is applied on the spot, no waiting", async () => {
  const { postId } = await claimed("size 95, kalau nggak ada 100 juga boleh", id("her-1"))

  const emoji = await trySizeOffer({
    groupJid: GROUP, messageId: "owner-1", sender: OWNER,
    text: "95 kosong kak, aku ambil 100 ya", quoted: id("her-1"),
  })
  assert.equal(emoji, "✅")
...
test("a size she never mentioned waits for her, changing nothing yet", async () => {
  const { postId } = await claimed("size 95 ya kak", id("her-2"))

  const emoji = await trySizeOffer({
    groupJid: GROUP, messageId: "owner-2", sender: OWNER,
    text: "95 kosong, adanya 100. Mau?", quoted: id("her-2"),
  })
  assert.equal(emoji, "❔")
...
test("her thumbs down leaves her at the size she asked for", async () => {
  const { postId } = await claimed("size 95 ya kak", id("her-3"))
  await trySizeOffer({
    groupJid: GROUP, messageId: "owner-3", sender: OWNER, text: "adanya 100", quoted: id("her-3"),
  })
...
test("somebody else's thumb on the offer does nothing", async () => {
  const { postId } = await claimed("size 95 ya kak", id("her-4"))
  await trySizeOffer({
    groupJid: GROUP, messageId: "owner-4", sender: OWNER, text: "adanya 100", quoted: id("her-4"),
  })
...
test("a customer cannot offer herself a size", async () => {
  const { postId } = await claimed("size 95 ya kak", id("her-5"))

  const emoji = await trySizeOffer({
    groupJid: GROUP, messageId: "hers-again", sender: HER, text: "100 aja deh", quoted: id("her-5"),
  })
...
test("a reply naming no size is not an offer", async () => {
  await claimed("size 95 ya kak", id("her-6"))
  const emoji = await trySizeOffer({
    groupJid: GROUP, messageId: "owner-6", sender: OWNER, text: "oke kak siap", quoted: id("her-6"),
  })
  assert.equal(emoji, null)
})

test("a message carrying three claims is refused rather than guessed at", async () => {
  const { postId } = await claimed("size 95", id("her-7"), ["size 95", "size 95"])

  const emoji = await trySizeOffer({
    groupJid: GROUP, messageId: "owner-7", sender: OWNER, text: "adanya 100", quoted: id("her-7"),
  })
...
test("the size she cannot have is not read as the offer", async () => {
  ...
  // be meaning.
  const { postId } = await claimed("size 95 ya kak", id("her-8"))
  await trySizeOffer({
    groupJid: GROUP, messageId: "owner-8", sender: OWNER,
    text: "95 kosong kak, adanya 100", quoted: id("her-8"),
  })
  await trySizeAnswer({
...
test("two sizes offered at once is a question, not an instruction", async () => {
  const { postId } = await claimed("size 95 ya kak", id("her-9"))
  const emoji = await trySizeOffer({
    groupJid: GROUP, messageId: "owner-9", sender: OWNER,
    text: "95 kosong, ada 100 atau 110", quoted: id("her-9"),
  })
  assert.equal(emoji, "😢")
...
test("repeating her own size back is not an offer at all", async () => {
  await claimed("size 95 ya kak", id("her-10"))
  const emoji = await trySizeOffer({
    groupJid: GROUP, messageId: "owner-10", sender: OWNER, text: "95 ya kak siap", quoted: id("her-10"),
  })
  assert.equal(emoji, null)
})
